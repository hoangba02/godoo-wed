...
import { Database } from './interfaces/indexDB';
...
export class IDB {
  ...
  public openDatabase(createNew = false): Promise<IDBDatabase> {
    if (this.openDbPromise && createNew === false) {
  ...
  public static create<T extends Database<any>>(db: T) {
    return (
      this.INSTANCES.find(instance => instance.name === db.name) ?? new IDB(db)
    );
  }
...
export default class IDBStorage<
  ...
  StoreName extends string = T['stores'][0]['name'],
> {
  ...
  public delete(
    ...
    if (!isArray) {
      entryName = ([] as any[]).concat(entryName);
    }

    return this.getObjectStore(
      'readwrite',
      objectStore => {
        const promises = (entryName as string[]).map(entryName =>
          objectStore.delete(entryName),
        );
  ...
  public add(entryName: string, value: any | any[], storeName: StoreName) {
    return this.getObjectStore(
      'readwrite',
      objectStore => objectStore.add(value, entryName),
      storeName,
    );
  }

  public put(entryName: string, value: any | any[], storeName: StoreName) {
    return this.getObjectStore(
      'readwrite',
      objectStore => objectStore.put(value, entryName),
      storeName,
    );
  }
  ...
  public save(
    ...
    return this.getObjectStore(
      'readwrite',
      objectStore => {
        const promises = (entryName as string[]).map((entryName, idx) =>
          objectStore.put(value[idx], entryName),
        );
        return isArray ? promises : promises[0];
      },
      storeName,
  ...
  public get<T>(
    ...
    if (!isArray) {
      if (!entryName) {
        return undefined;
      } else entryName = ([] as any[]).concat(entryName);
    } else if (!entryName.length) {
      return Promise.resolve([]) as any;
    }

    return this.getObjectStore<T>(
      'readonly',
      objectStore => {
        const promises = (entryName as string[]).map(entryName =>
          objectStore.get(entryName),
        );
        return isArray ? promises : promises[0];
      },
      storeName,
    ...
    return this.idb.openDatabase().then(db => {
      return new Promise<T>((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          console.log('transaction not finished', transaction);
        }, 10000);
        ...
        const requests: IDBRequest[] = isArray
          ? callbackResult
          : (([] as any[]).concat(callbackResult) as any);
        if (waitForTransactionComplete) {
          return;
        }
